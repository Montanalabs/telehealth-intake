...
# Telehealth intake router

Untrusted input is a chat intake; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a chat intake
- **Closed type:** `type Decision = Assign(Specialty) | Escalate` where `Specialty` is closed
- **Sink / capability:** `grant route` → `privileged { route(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/telehealth-intake/demo.sh
```
...
- `telehealth-intake_safe.wave` — the correct design.
- `telehealth-intake_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
